...
describe('Receiver', function() {
  ...
  describe('#parseMessage', function() {
    ...
    beforeEach(function() {
      receiver = new Receiver(fakeWindow);
      receiverSpy = this.sandbox.spy(receiver, 'parseMessage');
    });
  ...
  describe('#handleMessage', function() {
    ...
    beforeEach(function() {
      receiver.messageHandlers = {
        foo: this.sandbox.spy(),
        bar: this.sandbox.spy()
      };
      message = {
        event: 'foo',
        source: {
    ...
    it('should call message handler function if it is set', function() {
      ...
      expect(receiver.messageHandlers.foo).to.have.been.calledWith(message);
    });
    ...
    it('should trigger "foo" handler when receiving a "foo" event', function() {
      ...
      expect(receiver.messageHandlers.foo).to.have.been.calledWith(eventData);
    });
    ...
    it('should not trigger "bar" handler when receiving a "foo" event', function() {
      ...
      expect(receiver.messageHandlers.bar).to.have.callCount(0);
    });
  });
});
